...
- The CRLF case is covered end to end, against a fake vault, rather than by hand: **Obsidian rewrites a note's line endings to LF the first time it saves it**, so a note prepared for the purpose stops being the case under test as soon as anyone touches it. The hazard is real all the same — a linknote written from Reading view is often the first write such a note has had.
...
- **An emoji marker is one mark, however it is spelled.** A marker was measured in code points, and an emoji is regularly several — a zero-width joiner between the parts, a skin tone, a variation selector, the tag characters a flag is spelled out with. 🙆‍♀️ counts four that way, a family five, a flag seven. With the rule above in place, a marker written on a device whose marker is spelled with more than four could no longer be recognised from here, and the linknote could not be removed. Marks are counted as marks now.
